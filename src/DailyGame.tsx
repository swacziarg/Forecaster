...
function Help({ returnFocus, onClose }: { returnFocus: HTMLElement | null; onClose: () => void }) {
  return <Modal title="How to play" returnFocus={returnFocus} onClose={onClose}>
    ...
    <ol className="daily-rules">
      ...
      <li><span>2</span><p><strong>Put them in order.</strong> Top: outcome more likely. Bottom: outcome less likely. Hold a card to drag it, or use the arrows.</p></li>
...
function ScoringRules({ puzzle, returnFocus, onClose }: { puzzle: DailyPuzzle; returnFocus: HTMLElement | null; onClose: () => void }) {
  return <Modal title="Scoring rules" returnFocus={returnFocus} onClose={onClose}><ol className="daily-rules"><li><span>1</span><p><strong>Compare pairs.</strong> Each pair of headlines is checked against the market order. Your score is the percentage of comparable pairs you placed correctly, rounded to the nearest whole number.</p></li><li><span>2</span><p><strong>{puzzle.scoring.tieThreshold === 0 ? 'Use the full precision.' : 'Allow ties.'}</strong> {puzzle.scoring.tieThreshold === 0 ? 'Headlines are ranked by their unrounded market moves. Only equal calculated moves tie. Displayed values are rounded, so two matching labels can still have different ranks.' : <>Starting with the strongest remaining response, events within {formatThreshold(puzzle.scoring.tieThreshold)} of it form a tie group. The next group begins outside that range. Nearby values cannot chain together into a wider tie.</>}</p></li><li><span>3</span><p><strong>Skip what cannot be compared.</strong> Pairs within a tie group and events with insufficient data are excluded. If no comparable pairs remain, no score is calculated.</p></li></ol></Modal>
}
...
function Result({ study, puzzle, puzzles, nextPuzzle, impacts, submission, now, preview, onScoring, onStudy, onStats }: {
  ...
  return <main className="daily-result" id="daily-result"><header className="daily-result-hero"><span className="daily-result-kicker"><Check size={15} /> {preview ? 'Private preview complete' : submission.playMode === 'daily' ? 'Daily complete' : 'Archive complete'}</span><h1 ref={headingRef} tabIndex={-1}>{score === null ? '—' : score}<small>{score === null ? '' : '/100'}</small></h1><h2>{reaction}</h2><p>{scoreExplanation}</p>{(preview || submission.playMode === 'archive') && <p className="daily-archive-note">{preview ? 'Private preview results are saved separately from official daily stats and streaks.' : 'Archive results are saved separately from your current daily streak.'}</p>}<div className="daily-result-actions"><button className="daily-primary-button" onClick={share}><Share2 size={18} /> Share result</button><button className="daily-secondary-button" onClick={copy}><Copy size={18} /> Copy</button></div><div className="daily-share-preview" aria-label={`${filled} of 5 complete score bands`}><span>{tiles}</span><small>Each filled tile marks one complete 20-point band. The exact score is shared separately; answers are not.</small></div><p className="daily-share-feedback" role="status">{shareFeedback}</p></header>
    ...
    {study.conclusion && conclusionSource && <section className="daily-takeaway" id="daily-ending"><span>How it ended</span><h2>{study.conclusion.title}</h2><p>{study.conclusion.text}</p><a href={conclusionSource.url} target="_blank" rel="noreferrer">Original source <ExternalLink size={15} /></a></section>}
...
export function DailyGame({ registration, points, puzzle, playMode, nextPuzzle, puzzles, preview = false }: { registration: StudyRegistration; points: MarketSeriesPoint[]; puzzle: DailyPuzzle; playMode: 'daily' | 'archive'; nextPuzzle: DailyPuzzle | null; puzzles: readonly DailyPuzzle[]; preview?: boolean }) {
  ...
  const overlays = <>{modal === 'scoring' && <ScoringRules puzzle={puzzle} returnFocus={modalTrigger} onClose={() => setModal(null)} />}{modal === 'study' && <StudyDetails study={study} returnFocus={modalTrigger} onClose={() => setModal(null)} />}{modal === 'help' && <Help returnFocus={modalTrigger} onClose={closeHelp} />}{modal === 'stats' && <Stats puzzles={puzzles} returnFocus={modalTrigger} onClose={() => setModal(null)} />}</>
